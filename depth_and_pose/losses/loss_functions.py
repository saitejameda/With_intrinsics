...
import numpy as np
import cv2
from .inverse_warp import inverse_warp, inverse_warp_w1
...
class SSIM(nn.Module):
    ...
    def __init__(self):
        ...
        self.C1 = 0.01 ** 2
        self.C2 = 0.03 ** 2
#/home/meda/Thesis/Thesis_Project
...
def photo_and_geometry_loss(tgt_img, ref_imgs, tgt_depth, ref_depths, intrinsics, poses, poses_inv, hparams):
    ...
    if hparams.model_version == 'v3'or'w1'or'w2'or'L':
...
def compute_pairwise_loss(tgt_img, ref_img, tgt_depth, ref_depth, pose, intrinsic, hparams):
    if hparams.model_version == 'w1':
        ref_img_warped, projected_depth, computed_depth = inverse_warp_w1(
            ref_img, tgt_depth, ref_depth, pose, intrinsic, padding_mode='zeros', hparams=hparams)
    else:
...
def compute_smooth_loss(pred_disp,tgt_img):
    def gradient(pred):
        D_dy = pred[:, :, 1:, :] - pred[:, :, :-1, :]
        D_dx = pred[:, :, :, 1:] - pred[:, :, :, :-1]
        return D_dx, D_dy
    ...
    smooth_loss = torch.mean(torch.abs(dx2)) + \
...
#def compute_smooth_loss(tgt_depth, tgt_img):
    ...
    #     dy = pred[:, 1:, :, :] - pred[:, :-1, :, :]
    #     dx = pred[:, :, 1:, :] - pred[:, :, :-1, :]
    #     #dx = F.pad(dx, (0, 1, 0, 0))  # Pad to match the shape of depth_map
    #     #dy = F.pad(dy, (0, 0, 0, 1))  # Pad to match the shape of depth_map
    #     return dx, dy
    
    # dx, dy = gradient(depth_map)
    # dx2, dxdy = gradient(dx)
    # dydx, dy2 = gradient(dy)
    # dx2 = dx2[:, :, :, :-1]  # Trim the last dimension of dx2
    # #dy2 = dy2[:, :, :-1, :]  # Trim the last dimension of dy2
    # # Calculate the gradient of pixel intensity
    # #img_gray = torch.mean(img, dim=1, keepdim=True)
    # img_grad_x = torch.abs(img[:, :, :, :-1] - img[:, :, :, 1:])
    # img_grad_y = torch.abs(img[:, :, :-1, :] - img[:, :, 1:, :])
    # img_grad_x = img_grad_x[:, :, :-2, :]
    # #img_grad_y = img_grad_y[:, :, :-1, :]
    # print('img_grad_x shape: ', img_grad_y.shape)
    # print('dx2',dy2.shape)
    # #dx2 = F.pad(dx2, (0, 1, 0, 0))
    # #img_grad_x = F.pad(img_grad_x, (0, 1, 0, 0))
    # #img_grad_y = F.pad(img_grad_y,)
    
    
    # # Compute smoothness loss with exponential weighting
    # alpha = 1.0  # You can adjust this parameter as needed
    
    # smooth_loss = torch.mean(torch.abs(dx2) * torch.exp(-alpha * img_grad_x)) +torch.mean(
    #     torch.abs(dy2) * torch.exp(-alpha * img_grad_y))
    ...
    #     grad_disp_x_2 = torch.abs(grad_disp_x[:, :, :, :-1] - grad_disp_x[:, :, :, 1:])
    #     grad_disp_y_2 = torch.abs(grad_disp_y[:, :, :-1, :] - grad_disp_y[:, :, 1:, :])
        
    #     grad_img_x = torch.mean(
    #         torch.abs(img[:, :, :, :-1] - img[:, :, :, 1:]), 1, keepdim=True)
    #     grad_img_x = grad_img_x[:, :, :, :-1]
  
    #     grad_img_y = torch.mean(
    #         torch.abs(img[:, :, :-1, :] - img[:, :, 1:, :]), 1, keepdim=True)
    #     grad_img_y = grad_img_y[:, :, :-1, :]
    #     # grad_disp_x_2 = F.pad(grad_disp_x_2, (0, 0, 0, 1))
    #     # grad_disp_y_2 = F.pad(grad_disp_y_2, (0, 0, 1, 0))
    #     # print('grad_img_x shape: ', grad_img_x.shape)
    #     # print('grad_img_y shape: ', grad_img_y.shape)
    #     # print('grad_disp_x_2 shape: ', grad_disp_x_2.shape)
    #     # print('grad_disp_y_2 shape: ', grad_disp_y_2.shape)
        
    #     grad_disp_x_2 *= torch.exp(-grad_img_x)
    #     grad_disp_y_2 *= torch.exp(-grad_img_y)

    #     return grad_disp_x_2.mean() + grad_disp_y_2.mean()
