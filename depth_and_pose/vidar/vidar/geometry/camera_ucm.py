...
from vidar.vidar.utils.tensor import pixel_grid
...
class UCMCamera(nn.Module):
    ...
    def __init__(self, I, Tcw=None):
        ...
        if Tcw is None:
            ...
            self.Tcw = Tcw
            print('T.shape_isinstance',self.Tcw.T.squeeze().shape)
        else:
            self.Tcw = Pose(Tcw)
            print('T.shape',self.Tcw.T.squeeze().shape)
        #print('T.shape',self.Tcw.T.squeeze().shape)
    ...
    def reconstruct(self, depth, frame='w'):
        ...
        #print(fx.shape, fy.shape, cx.shape, cy.shape, alpha.shape)
    ...
    def project(self, X, frame='w'):
        ...
        B, C, H, W = X.shape
        assert C == 3
        #print("Tcw", self.Twc.detach())
        # Project 3D points onto the camera image plane
        if frame == 'c':
            X = X
        elif frame == 'w':
            #T = self.Twc
            #print('T_project',self.Twc.shape)
