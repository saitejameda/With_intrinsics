...
from  vidar.vidar.arch.networks.BaseNet import BaseNet
...
class IntrinsicsNet(BaseNet, ABC):
    ...
    def __init__(self):
        ...
        self.image_shape =  [256, 832]
        self.geometry =  'ucm'
        self.sigmoid_init = nn.Parameter(
            torch.tensor(self.setup_sigmoid_init(), dtype=torch.float), requires_grad=True)
        self.scale = nn.Parameter(
            torch.tensor(self.setup_scale(), dtype=torch.float, requires_grad=False), requires_grad=False)
        self.offset = nn.Parameter(
            torch.tensor(self.setup_offset(), dtype=torch.float, requires_grad=False), requires_grad=False)
